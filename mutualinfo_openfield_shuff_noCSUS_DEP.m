function f = mutualinfo_openfield_shuff_noCSUS_DEP(spike_structure, pos_structure, velthreshold, dim, CA_timestamps, CSUS_id_struct, ca_MI, num_times_to_run)
          ...
          for k=1:numunits
                ...
                parfor l = 1:num_times_to_run
                      ...
                      if isnan(MI(k))==0 && length(highspeedspikes)>1
                        ...
                        %pos shuffle
                        %shuffled_idx = randperm(size(pos_only,1));
                        %shiftedData = pos_only(shuffled_idx,:);
                        ...
                        if nPos < 9
                            ...
                            shiftedData = circshift(pos_only, shift, 1);   % rows only
                            shuff_pos   = [timevec, shiftedData];
                        end
                        % end circular shift
