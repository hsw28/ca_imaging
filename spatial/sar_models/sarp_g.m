...
%          results.time3  = time for MCMC sampling
%          results.time4  = time for effects estimates
...
% written by:
% James P. LeSage, last updated 1/2011
...
% compute effects estimates after MCMC loops to save time
...
hwait = waitbar(0,'sarp: calculating effects ...');
t0 = clock;                  
...
for ii=1:ndraw;
    ...
    rho = pdraws(ii,1);
          ...
          avg_direct(kk,ii) = (pdfz'*estimated_diags*rhovec)*bdraws(ii,kk)/n; %av direct effect
          avg_total(kk,ii) = mean(sum(dd*s*bdraws(ii,kk),2));
          avg_indirect(kk,ii) = avg_total(kk,ii) - avg_direct(kk,ii);

          % observation-level total effects
          totals = sum(dd*s*bdraws(ii,kk),2);
