function f = mutualinfo_CSUS_trace_shuff15(spike_structure, CSUS_structure, do_you_want_pretrial, num_times_to_run, MI_CSUS)
%SHUFFLES CSUS
        ...
        for k=1:size(peaks_time,1)
          currspikes = peaks_time(k,:);
              ...
              if length(currspikes)>0 &&  isnan(MI(k))==0 && length(unique(CSUS))>=3 %finding how many spikes in each time bin
                ...
                variableName = sprintf('MI_CSUS5_trace_shuff15%s', suffix);

                % Assign the structure to the new variable name
                eval([variableName ' = MI_CSUS5_trace_shuff15;']);
