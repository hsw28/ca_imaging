function mutualinfo_struct = MI_control_matchSpikes(spike_structure, pos_structure, velthreshold, dim, CA_timestamps, CSUS_id_struct, ca_MI, nIter)
...
for i = 1:numel(fields_spikes)
    ...
    parfor iIter = 1:nIter
        for k = 1:nCells
            ...
            CSUS_spikes = spk((spike_vel >= velthreshold) & (csus_spk > 0));

            if numel(valid_spikes) >= 1
                resampled = randsample(all_spikes, numel(all_spikes)-numel(CSUS_spikes), false);
