function K = compute_alpha_kernel_sparse(data, ...
    progress_callback, varargin)
...
feval(progress_callback, 'Computing alpha decay kernel:');

N = size(data, 1); % number of cells

if ~isempty(npca)
    feval(progress_callback, '   PCA');
...
feval(progress_callback, ['Number of samples = ' num2str(N)])

% Initial knn search and set the radius
feval(progress_callback, ['First iteration: k = ' num2str(k_knn)])
...
feval(progress_callback, ['Number of samples below the threshold from 1st iter: ' num2str(length(idx_thresh))])
...
while length(idx_thresh)<.9*N
    ...
    feval(progress_callback, ['Next iteration: k= ' num2str(k_knn)])
...
%         Add the newly thresholded points to the old ones
    ...
    feval(progress_callback, ['Number of samples below the threshold from the next iter: ' num2str(length(idx_thresh))])
end

% Radius search for the rest
if length(idx_thresh)<N
    feval(progress_callback, ['Using radius based search for the rest'])
    data_pc2=data_pc(~below_thresh,:);
    epsilon2=epsilon(~below_thresh);
...
feval(progress_callback, '   Symmetrize affinities');
K = K + K';
feval(progress_callback, '   Done computing kernel');
