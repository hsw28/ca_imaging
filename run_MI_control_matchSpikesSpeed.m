function run_MI_control_matchSpikesSpeed(csusNum)
...
for i = 1:numel(ratNames)
    ...
    % Run control shuffle
    out = MI_control_matchSpikesSpeed(spikes, pos, 4, dim, ts, csus, MI, nIter);

    % Store in struct
    fieldname = sprintf('MI_noCSUS%d_controlSpeed', csusNum);
    rat.(fieldname) = out;
