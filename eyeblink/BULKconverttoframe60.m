function CS_US_id_struct = BULKconverttoframe60(US_time_structure, Ca_timestamps)
  ...
  %    CS_timestoconvert = CS_time_structure.(field_name);
      ...
      date = field_name(index(2)+1:end)
            ...
            %  end
                ...
                if US_frame-16>0 && US_frame+53<=length(allframes)
                  if US_frame-16>1 <= length(allframes)
                    ...
                    allframes(US_frame+0:US_frame+53)=[7:60];
                ...
                elseif US_frame+53>length(allframes) && length(allframes(US_frame:allframes(end)))>0
